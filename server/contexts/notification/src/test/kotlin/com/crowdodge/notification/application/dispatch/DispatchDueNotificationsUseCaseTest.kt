...
class DispatchDueNotificationsUseCaseTest : FunSpec({
    ...
    fun due(kind: NotificationKind = NotificationKind.Reminder) =
        NotificationSchedule.schedule(userUuid, eventUuid, kind, NOW - 1.minutes)
    ...
    test("同一 eventUuid のスケジュールが複数あっても読み取りは各 1 回") {
        ...
        val result = DispatchDueNotificationsUseCase(
            ...
            congestions = {
                congestionCalls += 1
                mapOf(eventUuid to CongestionInfo("混雑中"))
            },
            sender = sender,
            transactions = ImmediateTransactionRunner,
            clock = FixedClock,
    ...
    test("複数スケジュールでも送信は sendAll 1 回にまとめる") {
        ...
        repo.save(due())
        repo.save(NotificationSchedule.schedule(otherUser, eventUuid, NotificationKind.Reminder, NOW - 1.minutes))
        val sender = RecordingSender()
        val readModel = FakeReadModel(
            sources = mapOf(eventUuid to source(eventUuid)),
            tokens = mapOf(
    ...
    test("複数スケジュールでも保存は claim と結果の saveAll 各 1 回") {
        val repo = InMemoryScheduleRepository()
        val schedule1 = due()
        val schedule2 = due(NotificationKind.CongestionAlert)
        repo.save(schedule1)
        repo.save(schedule2)
        val readModel = FakeReadModel(
            sources = mapOf(eventUuid to source(eventUuid)),
    ...
    test("completed / canceled / failed が混在する同一バッチを正しく確定する") {
        ...
        val failedSchedule =
            NotificationSchedule.schedule(noTokenUser, eventUuid, NotificationKind.Reminder, NOW - 1.minutes)
        repo.save(completedSchedule)
        repo.save(canceledSchedule)
        repo.save(failedSchedule)
        val sender = RecordingSender()
        val readModel = FakeReadModel(
            // canceledEvent は source なし → canceled、noTokenUser はトークンなし → failed
            sources = mapOf(eventUuid to source(eventUuid)),
            tokens = mapOf(userUuid to listOf("token-1")),
        )
